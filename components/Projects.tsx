...
const Projects: React.FC = () => {
  ...
  const categories: { id: Category; label: string; icon: any }[] = [
    { id: 'All', label: t('projects.filterAll'), icon: Filter },
      ...
      <div className="container mx-auto px-4 md:px-6">
        <ScrollReveal>
          <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-6">
            <div>
              <h2 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white mb-2">{t('projects.title')} <span className="text-brand-500">{t('projects.titleHighlight')}</span></h2>
              ...
              <ScrollReveal key={`${project.title}-${index}`} delay={index * 100}>
                <div className="group bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden hover:shadow-xl dark:hover:shadow-2xl dark:hover:shadow-brand-500/10 transition-all duration-300 hover:-translate-y-2 h-full flex flex-col">
                  <div className="relative h-48 overflow-hidden flex-shrink-0">
                    <img 
                      src={project.image} 
                      alt={language === 'vi' ? project.title : (project.title_en || project.title)} 
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2 group-hover:text-brand-500 transition-colors">
                      {language === 'vi' ? project.title : (project.title_en || project.title)}
                    </h3>
                    <p className="text-slate-600 dark:text-slate-400 text-sm mb-4 line-clamp-2 flex-1">
                      {language === 'vi' ? project.description : (project.description_en || project.description)}
                    </p>
                    ...
                    <div className="flex gap-4 mt-auto">
                      ...
                      {project.link && project.link !== '#' && (
                        <a href={project.link} target="_blank" rel="noreferrer" className="flex-1 flex items-center justify-center gap-2 py-2 bg-brand-600 hover:bg-brand-500 text-white rounded-lg text-sm font-medium transition-colors shadow-brand-500/20 shadow-lg hover:shadow-brand-500/40">
        ...
        <div className="mt-10 text-center">
            <a href="https://github.com/kzi207" target="_blank" className="inline-flex items-center gap-2 text-brand-600 dark:text-brand-400 hover:underline font-medium">
                {t('projects.more')} <ExternalLink size={14} />
            </a>
        </div>
      </div>
    </section>
