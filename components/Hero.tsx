...
const Hero: React.FC<HeroProps> = ({ onOpenCV }) => {
  ...
  const fullText = language === 'vi' ? PERSONAL_INFO.title : (PERSONAL_INFO.title_en || PERSONAL_INFO.title);
  ...
  useEffect(() => {
    ...
    const typeNextChar = () => {
      if (currentIndex < fullText.length) {
        ...
        currentIndex++;
        timeoutId = setTimeout(typeNextChar, 40 + Math.random() * 30); 
      }
    };
        ...
        <div className="space-y-8">
            ...
            <div className="space-y-4">
                <div className="bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm p-1 rounded-xl inline-flex gap-1 border border-slate-200 dark:border-slate-700">
                    {ABOUT_TABS.map((tab) => (
                        ...
                        >
                            {getTabIcon(tab.id)}
                            {language === 'vi' ? tab.label : (tab.label_en || tab.label)}
                        </button>
                    ))}
                </div>
                    ...
                    {ABOUT_TABS.map((tab) => (
                        activeTab === tab.id && (
                            <p key={tab.id} className="text-slate-600 dark:text-slate-300 max-w-lg leading-relaxed animate-blur-in text-lg">
                                {language === 'vi' ? tab.content : (tab.content_en || tab.content)}
                            </p>
                        )
                    ))}
                </div>
          ...
          <ScrollReveal variant="up" delay={400} immediate>
            <div className="flex flex-wrap gap-4 pt-2">
              ...
              >
                <FileText size={18} className="group-hover:text-brand-500 transition-colors" /> {t('hero.viewCV')}
              </button>
            </div>
          </ScrollReveal>
